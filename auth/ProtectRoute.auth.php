...
class ProtectRoute {
    ...
    public function __construct() {
        ...
        if (isset($_COOKIE['access_token']) && isset($_COOKIE['user_token'])) {
            ...
            if (isset($_SESSION['user_email']) && isset($_SESSION['user_title'])) {
            
                // Get the email and role
                $this->email = $_SESSION['user_email'];
                $this->role = $_SESSION['user_title'];
            ...
            // If the session is not exist,
                ...
                $this->title = hex2bin($arr_user_token[1]);

                $access_token_cpy = $this->rand_uuid . ":" . hash('crc32', $this->rand_uuid . $this->email) . ":" . hash('crc32', $this->rand_uuid . $this->title);
    ...
    public function validateClerk() {

        if ($_SESSION['user_title'] != "Clerk") {
            $homeurl = "./" . strtolower($_SESSION['user_title']) . ".php?tab=home&section=main";
            forbidden($homeurl);
            return false;
        }
    ...
    public function validateTeacher() {
        ...
        if ($_SESSION['user_title'] != "Teacher") {
            $homeurl = "./" . strtolower($_SESSION['user_title']) . ".php?tab=home&section=main";
            forbidden($homeurl);
            return false;
        }
    ...
    public function validateManager() {
        ...
        if ($_SESSION['user_title'] != "Manager") {
            $homeurl = "./" . strtolower($_SESSION['user_title']) . ".php?tab=home&section=main";
            forbidden($homeurl);
            return false;
        }
